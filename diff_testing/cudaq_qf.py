...
def _apply_opt_level_1(kernel) -> None:
    ...
    """
    cudaq.passes.aggressive_early_inlining(kernel)
...
def _apply_opt_level_2(kernel) -> None:
    ...
    """
    cudaq.passes.aggressive_early_inlining(kernel)
    cudaq.passes.unitary_synthesis(kernel)
...
def _apply_opt_level_3(kernel) -> None:
    ...
    cudaq.passes.aggressive_early_inlining(kernel)
    cudaq.passes.unitary_synthesis(kernel)
    cudaq.passes.decompose_to_basis(kernel)
...
class cudaqTesting(Base):
    ...
    def get_counts(self, kernel, opt_level: int, circuit_num: int) -> Dict[Any, int]:
        ...
        try:
            ...
            result: cudaq.SampleResult = cudaq.sample(kernel, shots_count=self.num_shots)
