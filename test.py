...
from pytket.circuit import CircBox, Unitary1qBox, Unitary2qBox, if_bit, if_not_bit
from pytket.extensions.qiskit.backends.aer import AerBackend
from pytket.passes import DecomposeBoxes, DecomposeMultiQubitsCX, DefaultMappingPass

raw = [
    (-0.61084784968805961, -0.63634846256633193),
    (-0.11707709961741494, -0.45630964428347692),
    (0.17864395362519447, 0.4359035174830535),
    (0.10984380022082535, -0.87521928728296883),
]
U = np.array([complex(r, i) for r, i in raw], dtype=complex).reshape(2, 2)
unitary_0 = Unitary1qBox(U)

raw = [
    (-0.59269898070373939, -0.072768811419844051),
    (-0.19452998640440455, 0.7781842344501676),
    (0.79640915840373239, 0.095629863368878132),
    (-0.14327733711076041, 0.57970594814367571),
]
U = np.array([complex(r, i) for r, i in raw], dtype=complex).reshape(2, 2)
unitary_1 = Unitary1qBox(U)

raw = [
    (0.83718046651317779, -0.29972949141358551),
    (0.40284658897818199, -0.11444883938267848),
    (-0.049785034049742181, -0.14416660937238707),
    (-0.10052914427508081, -0.023205983202368184),
    (-0.044256502308110943, -0.16133491603136704),
    (0.029668545405364694, 0.39281378869690531),
    (0.31440612040087262, -0.3696124440033669),
    (-0.017460578359127222, 0.76227296462055616),
    (0.00082006131702854466, 0.061440765077563832),
    (0.050348760331332044, -0.48328575937445012),
    (0.80396621107586119, -0.077462498475337788),
    (0.31206293224659143, -0.10187667103424945),
    (0.23703438164080326, 0.34834260024380775),
    (-0.35279043488468437, -0.55576252114419245),
    (-0.28913595957125227, 0.072877715936758181),
    (0.06751971721125892, 0.54375489617219519),
]
...
raw = [
    (-0.2437552693330888, 0.15019670186050055),
    (0.70861410171565542, 0.47004603631784803),
    (-0.16633256284749026, -0.054713417954879336),
    (-0.045700950079130345, -0.40273867657368267),
    (-0.056221074610824012, -0.47925588822291548),
    (-0.15572586567849925, -0.35068825612760812),
    (-0.29978099674812142, 0.037027376208342941),
    (0.09851780662054524, -0.72039902507196185),
    (-0.28315040883071518, -0.61400226271389291),
    (0.23443085405423636, -0.0075739165090283238),
    (-0.42268467149823064, -0.072326215007531156),
    (-0.10123690073885079, 0.54191286647323966),
    (0.26020431543166511, -0.4003724333515068),
    (0.21319676434625351, 0.17094423211345139),
    (0.38528146798416507, 0.73875911186474919),
    (0.055766853273586707, 0.0020152327653041322),
]
U = np.array([complex(r, i) for r, i in raw], dtype=complex).reshape(4, 4)
